...
```sh
python wdv3_timm.py path/to/image.png --model <vit|swinv2|convnext>
```

- 第1引数（位置引数）`image_file` は画像パス
- `--model`（デフォルト `vit`。`vit`/`swinv2`/`convnext` から選択。`wdv3_timm.py` 内 `MODEL_REPO_MAP` で Hugging Face リポジトリ ID にマッピング）
